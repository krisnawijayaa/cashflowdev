...
// ─── RENDER UTANG (SUMMARY + PIUTANG + MY DEBT HISTORY) ──────────────────────
...
// ─── HISTORI PIUTANG (unchanged full log — every transaction, paid or not) ───
function renderPiutang(){
  ...
  Array.from(selectedDebtors).forEach(k=>{ if(!selectableDebtors.has(k) && map[k]===undefined) return; });
